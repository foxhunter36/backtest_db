"""
sol_train_model.py – Trainiert ML Filter für den SOL Supertrend Bot.

Klassifiziert: "Wird dieser Supertrend Trade profitabel sein?"

Usage:
    python sol_train_model.py
    python sol_train_model.py --model rf
    python sol_train_model.py --symbol SOLUSDT --timeframe 1h
"""
...
from sol_feature_engineering import prepare_dataset

OUTPUT_DIR = Path(__file__).parent / "models"
OUTPUT_DIR.mkdir(exist_ok=True)


def train(symbol="SOLUSDT", timeframe="1h", model_type="xgb"):
    print("=" * 60)
    print(f"  SOL SUPERTREND ML FILTER TRAINING")
    print(f"  Symbol: {symbol} | Timeframe: {timeframe} | Model: {model_type}")
    print("=" * 60)

    X, y, df, feature_names = prepare_dataset(symbol=symbol, timeframe=timeframe)
    ...
    pkg = {
        ...
        "train_date": ts, "train_samples": len(X_train),
        "test_accuracy": accuracy_score(y_test, y_pred),
        "test_precision": precision_score(y_test, y_pred, zero_division=0),
        "cv_score_mean": np.mean(cv_scores),
        "strategy": "sol_supertrend_ao",
    }
    model_path = OUTPUT_DIR / f"sol_filter_{model_type}_{timeframe}_{ts}.pkl"
    joblib.dump(pkg, model_path)
    joblib.dump(pkg, OUTPUT_DIR / "sol_filter_latest.pkl")
...
if __name__ == "__main__":
    ...
    train(symbol=args.symbol, timeframe=args.timeframe, model_type=args.model)
